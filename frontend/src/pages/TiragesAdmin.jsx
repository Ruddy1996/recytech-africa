...
export default function TiragesAdmin() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const { data, headers } = await axiosInstance.get("/tirages", {
  ...
  const create = async () => {
    try {
      await axiosInstance.post("/tirages", form);
  ...
  const toggleActif = async (id, actif) => {
    try {
      await axiosInstance.patch(`/tirages/${id}/actif`, { actif: !actif });
      setRows((r) => r.map((t) => (t.id === id ? { ...t, actif: !actif } : t)));
    } catch {
      toast.error("Échec modification statut");
    }
  };
  ...
  const draw = async (id) => {
    ...
    try {
      await axiosInstance.post(`/tirages/${id}/draw`);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <header className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => setPage((p) => p)} /* juste rafraîchir */
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4">
            <Dialog.Panel className="bg-white w-full max-w-lg rounded-lg p-6 shadow-lg">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <textarea
                  className="col-span-2 border px-3 py-2 rounded h-24 resize-none"
                ...
                <input
                  ...
                  min="0"
                  placeholder="Points"
                  value={form.points_participation}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      points_participation: Number(e.target.value),
                    })
                  }
                />
